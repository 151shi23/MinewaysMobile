 ...
 */
public class BedrockLegacyItemStackResolver extends ItemStackResolver<BedrockResolvers, CompoundTag> {
    /**
    ...
    /**
     * Create a new legacy bedrock item stack.
     *
        ...
        registerHandler(ChunkerItemProperty.DISPLAY, new PropertyHandler<>() {
            ...
            public Optional<ChunkerItemDisplay> read(@NotNull CompoundTag value) {
                ...
                if (tag.contains("customColor")) {
                    color = ChunkerColor.fromRGB(tag.getInt("customColor"));
                }
            ...
            public Optional<ChunkerFireworks> read(@NotNull CompoundTag value) {
                ...
                if (explosionsTag != null) {
                    ...
                    for (CompoundTag explosionTag : explosionsTag) {
                        ...
                        List<ChunkerColor> colors = colorsRGB == null ? Collections.emptyList() : mapFireworkColors(colorsRGB);

                        // Parse fade colors
                        byte[] fadeColorsRGB = explosionTag.getByteArray("FireworkFade", null);
                        List<ChunkerColor> fadeColors = fadeColorsRGB == null ? Collections.emptyList() : mapFireworkColors(fadeColorsRGB);
     ...
     */
    public List<ChunkerColor> mapFireworkColors(byte[] colorIndexes) {
        List<ChunkerColor> colors = new ArrayList<>(colorIndexes.length);
        ...
        for (byte colorIndex : colorIndexes) {
            ...
            colors.add(ChunkerColor.fromRGB(dyeColor.getJavaRGB()));
        }
        return colors;
    }
     ...
     */
    public byte[] mapFireworkColors(List<ChunkerColor> colors, boolean defaultBlack) {
    ...
    protected Optional<CompoundTag> createOutput(ChunkerItemStack input) {
        ...
        if (itemIdentifier.isPresent()) {
            ...
            // Used the fixed name as otherwise it doesn't load properly
