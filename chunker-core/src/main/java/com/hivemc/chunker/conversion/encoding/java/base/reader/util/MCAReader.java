...
public class MCAReader implements AutoCloseable {
    ...
    protected CompoundTag decompressColumn(ChunkCoordPair chunkCoordPair, byte compressionType, byte[] compressedColumn) throws IOException {
        try {
            ...
            // Android 兼容：String.formatted() 为 Java 15 新增，旧 Android 上没有，改用 String.format
            throw new IOException(String.format("Failed to decompress column %s inside %s", chunkCoordPair, chunkCoordPair.getRegion()), e);
        }
    }
